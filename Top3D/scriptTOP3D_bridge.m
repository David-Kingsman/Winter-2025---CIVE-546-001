...
[Il,Jl] = meshgrid(1:nelx+1,1:nely+1);                  % Coordinates
Fl = -ones(size(Il));
Fl([1 end],:)=Fl([1 end],:)/2;
Fl(:,[1 end])=Fl(:,[1 end])/2;
loadnid = nelz*(nelx+1)*(nely+1)+(Il-1)*(nely+1)+Jl;    % Node IDs
loaddof = 3*loadnid(:);                                 % DOFs
loadval = Fl(:);
% USER-DEFINED SUPPORT FIXED DOFs
[If,Jf] = meshgrid(1:round(nelx/12),1:nely+1);          % Coordinates
fixednid1 = (If-1)*(nely+1)+Jf;                         % Node IDs
[If,Kf] = meshgrid(1:nelx+1,1:nelz+1);                  % Coordinates
fixednid2 = (If-1)*(nely+1)+(Kf-1)*(nelx+1)*(nely+1)+1; % Node IDs
[Jf,Kf] = meshgrid(1:nely+1,1:nelz+1);                  % Coordinates
fixednid3 = nelx*(nely+1)+(Kf-1)*(nelx+1)*(nely+1)+Jf;  % Node IDs
fixeddof = [3*fixednid1(:); 3*fixednid1(:)-1; 3*fixednid1(:)-2; ...
            3*fixednid2(:)-1; 3*fixednid3(:)-2];        % DOFs
...
pass_solid((Kp-1)*nelx*nely+(Ip-1)*nely+Jp) = true;
